  ...
  switch (action.type) {
      ...
      if (
        ...
        if (action.newMessage.recieverId === state.userInfo.id) {
          ...
          if (index !== -1) {
            ...
            data.messageTypeId = action.newMessage.messageTypeId;
            data.messageId = action.newMessage.id;
            data.messageStatusId = action.newMessage.messageStatusId;
    ...
    case reducerCases.SET_MESSAGES:
      ...
      case reducerCases.DELETE_MESSAGE: {
        console.log("delete")
        const clonedMessages = state.messages.filter(
        ...
        const index = clonedContacts.findIndex(
          (contact) =>
            contact.id === state.currentChatUser.id ||
            contact.id === action.senderId
        );
        if (index !== -1) {
          const data = clonedContacts[index];
          clonedContacts.splice(index, 1);
    ...
    case reducerCases.SET_MESSAGES_READ: {
      if (state.userInfo.id === action.id) {
        ...
        clonedMessages.forEach(
          (msg, index) => (clonedMessages[index].messageStatusId = 3)
        );
        const index = clonedContacts.findIndex(
          (contact) => contact.id === action.recieverId
        );
        if (index !== -1) {
          clonedContacts[index].messageStatusId = 3;
        }
        return {
          ...state,
          messages: clonedMessages,
