              ...
              {isContactPage ? (
                ...
                <div className="flex items-center gap-1 max-w-[200px] sm:max-w-[250px] md:max-w-[300px] lg:max-w-[200px] xl:max-w-[300px]">
                  {data.senderId === userInfo.id && (
                    <MessageStatus messageStatusId={data.messageStatusId} />
                  )}
                  {data.messageTypeId === 1 && (
                    <span className="truncate ">{data.message}</span>
                  )}
                  {data.messageTypeId === 3 && (
                    <span className="flex gap-1 items-center">
                      <FaMicrophone className="text-panel-header-icon" />
                      Аудио
                    </span>
                  )}
                  {data.messageTypeId === 2 && (
                    <span className="flex gap-1 items-center">
                      <FaCamera className="text-panel-header-icon" />
                      Изображение
                    </span>
                  )}
                  {data.messageTypeId === 4 && (
                    <span className="flex gap-1 items-center">
                      <FaCopy className="text-panel-header-icon" />
                      Файл
                    </span>
                  )}
                  {data.messageTypeId === 6 && (
                    <span className="flex gap-1 items-center">
                      <FaFileArchive className="text-panel-header-icon" />
                      Архив
                    </span>
                  )}
                  {data.messageTypeId === 5 && (
                    <span className="flex gap-1 items-center">
                      <FaPlay className="text-panel-header-icon" />
                      Мультимедиа
                    </span>
                  )}
                </div>
